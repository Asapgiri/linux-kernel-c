...
\newcommand{\ascode}[1]{{\fontfamily{DejaVu Sans Mono}\ttfamily\selectfont\replunderscores{#1}}}
...
\begin{frame}
    ...
    Bitwise operations execute the following on eather one or two different variables:
...
\begin{frame}[fragile]
    ...
    \alert{They shouldn't contain function declarations!}

    \bigskip

    You will also see \ascode{\#include <stdio.h>}.
    The difference between refering to an include with \ascode{""} and \ascode{<>} is that
...
\begin{frame}[fragile]
    ...
    Strings are an array of characters. They can eather be stored in an "array", or a pointer.
